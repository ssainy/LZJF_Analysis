...
def get_score(rank, weights=[]):
    ...
    rank['bank_creditlimit'] = rank['grade'].map({'A': 2000000, 'B': 500000, 'C': 300000, 'D': 0, 'E': 0})
    return rank



order = pd.read_sql_query("select * from tra_purchase_order where ordering_time > DATE_SUB(CURDATE(), INTERVAL 2 YEAR) ".format(), engine)
order = order[
...
order = pd.read_sql_query("select * from tra_sales_order where ordering_time > DATE_SUB(CURDATE(), INTERVAL 2 YEAR) ", engine)
order = order[
        [u'order_header_code', u'buyer_id', u'product_name', u'unit_price', u'ordering_quantity', u'ordering_time',
         u'discount_money', u'coupon_money',u'seller_stock_change_time', u'seller_stock_change_quantity', u'buyer_stock_change_time',
         u'buyer_stock_change_quantity', u'order_status', u'total_money', u'seller_id']]

order.columns = ['order_ID', 'company_id', 'type_of_merchandize', 'unit_price', 'order_num', 'order_time',
                     'discount_money', 'coupon_money','send_time', 'send_num', 'receive_time', 'receive_num', 'order_status', 'pay_amt',
                     'saler_id']
order['order_time'] = order['order_time'].apply(lambda x: convert_time(x))
order['send_time'] = order['send_time'].apply(lambda x: convert_time(x))
order['receive_time'] = order['receive_time'].apply(lambda x: convert_time(x))
...
# 优惠金额
tmp = order_finish[order_finish['sale_discount_amt'] > 0].groupby(['saler_id'])['sale_discount_amt'].sum().to_frame()
