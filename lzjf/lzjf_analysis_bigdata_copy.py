...
buyer_id = pd.read_sql_query("select buyer_id  from tra_purchase_order where  ordering_time > DATE_SUB(CURDATE(), INTERVAL 2 YEAR) group by buyer_id ;",engine)
...
for buyer_id in b:
    ...
    buy_order = pd.read_sql_query(sql.format(buyer_id), engine)
    print("--------------采购订单------------------")
    print(buy_order)
    ...
    sale_order = pd.read_sql_query("select * from tra_sales_order where seller_id in ({}) and ordering_time > DATE_SUB(CURDATE(), INTERVAL 2 YEAR) ;".format(buyer_id), engine)
    print("--------------销售订单------------------")
    print(sale_order)
